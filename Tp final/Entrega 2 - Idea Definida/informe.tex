...
En la búsqueda para dar con un conjunto de \textit{semillas} que sea una buena aproximación a centroides, proponemos analizar los siguientes métodos (ver \cite{Forgy-kmeans, original, k-means++}):
\begin{itemize}
\item McQueen (1967) 
\item Etiquetado 
\item McRae (1971) 
...
A los efectos de establecer la calidad de los clusters encontrados por el algoritmo \textit{k-means clustering standard}, usando la inicialización de los distintos algoritmos propuestos, proponemos 4 medidas posibles (ver \cite{Andrea-Guillermo, SchulteImWalde:03}): 

\begin{itemize}
\item \textbf{Distancia Intra-Cluster} (cohesión). Distancia entre todos los elementos de un cluster, el objetivo es minimizar esta distancia:
\begin{align*}
\frac{\sum_{i=1}^{K} (\sum_{z,t \in C_i} d(z, t) / \left | C_i \right |)}{K}
\end{align*}

\item \textbf{Distancia Inter-Cluster} (separación). Distancia entre los centroides de los clusters, donde el objetivo es maximizar la distancia entre ellos:
...
\item \textbf{Pureza}. A cada cluster se le asigna una clase, la cual es la más frecuente en el cluster, y la medida se realiza contando la cantidad de elementos que pertenecen a esa clase dentro del cluster dividido la totalidad de elementos en el cluster. Formalmente:

\begin{align*}
\mbox{purity}( \Omega,\mathbb{C} ) = \frac{1}{N} \sum_k \max_j \vert\omega_k \cap c_j\vert.
\end{align*}

\item \textbf{Matriz de confusión o tabla de contingencia}. A partir de la información brindada por la misma podemos extraer indices como el RI (Rand index), F score entre otras.
...
\begin{itemize}
\item \textit{Iris Plants}: es una base de datos con $4$ atributos numéricos, $3$ clases y $150$ elementos. La misma fue proporcionada por la cátedra.
\item \textit{Wine Quality}: una base de datos para clasificar vinos según su calidad, obtenida de \textit{UCI Machine Learning Repository}. La misma se divide en casos casos para \textit{Vino Blanco} y \textit{Vino Tinto}, con $4899$ y $1600$ casos respectivamente. Estas casos contienen $12$ atributos de los cuales los primeros $11$ son características fisico-químicas, mientras que el último atributo representa la calidad exacta del vino.
\item \textit{Glass Identification}: también obtenida de \textit{UCI Machine Learning Repository} y contiene $214$ casos con $10$ atributos cada uno. Ésta base de datos contiene 6 tipos de vidrio definidos en términos de su contenido de óxido (es decir $Na$, $Fe$, $K$, etc).
\end{itemize}
